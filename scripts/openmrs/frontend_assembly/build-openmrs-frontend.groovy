...
if (frontendCustomizationsFile.exists()) {
    frontendCustomizationsConfig = slurper.parse(frontendCustomizationsFile)
    // Get excluded frontend modules from the customizations file
    def excludedFrontendModules = frontendCustomizationsConfig["excludedFrontendModules"] ?: []

    // Remove excluded modules from reference application config
    excludedFrontendModules.each { module ->
        if (refAppConfig.frontendModules?.containsKey(module)) {
            log.info("Removing '${module}' from the reference application configuration...")
            refAppConfig.frontendModules.remove(module)
            refAppConfigFile.text = JsonOutput.prettyPrint(JsonOutput.toJson(refAppConfig))
            log.info("Removed '${module}' from the configuration.")
        }
    }
}
